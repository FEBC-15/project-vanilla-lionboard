...
- COLLECTIONS > Create Collection
  - Name: `test-brunch`
    - 팀 프로젝트에 적용할 경우 `team-brunch` 또는 `team-nike`
  - Location: 프로젝트루트/api/bruno
...
- test-brunch(`team-brunch`, `team-nike`) Collection 선택
  - 팀 프로젝트에 적용할 경우 team-brunch 또는 team-nike Collection 선택
...
- Bruno 우측 상단의 No Environment > Create
  ...
  - Add variable
    - Name: `client-id`
    - Value: `brunch`
      - 팀 프로젝트에 적용할 경우 `febc15-vanilla팀번호-ecad`. 5팀 예시, `febc15-vanilla05-ecad`
  - Save

#### 게시물 목록 조회 테스트

- test-brunch(`team-brunch`, `team-nike`) 컬렉션에 마우스 올린 후 더보기(...) > New Request
  - Type: HTTP
...
- test-brunch(`team-brunch`, `team-nike`) 컬렉션에 마우스 올린 후 더보기(...) > Settings > Headers > Add Header
  - Name: `client-id`
  - Value: `{{client-id}}`
  - Save
...
- test-brunch(`team-brunch`, `team-nike`) 컬렉션에 마우스 올린 후 더보기(...) > Settings
  - Script 탭의 Post Response에 추가 후 Save
...
### 5.1 팀 프로젝트 DB 초기화
- 팀 프로젝트 폴더에서 [3. DB 초기화] 작업 수행

### 5.2 팀 프로젝트 API 서버 테스트
- 팀 프로젝트 폴더에서 [4. API 서버 테스트] 작업 수행 후 push

#### 팀 레포지토리에 반영
- 수행한 작업에 대한 PR 생성 후 push

#### 담당 기능에 대한 API 테스트
- 팀원은 pull 받은 후 담당 기능에 대한 API 테스트 추가
  - api/bruno/readonly-brunch, api/bruno/readonly-nike 참고

### 5.3 .gitignore 파일에 readonly 폴더 추가
- 제공된 참고 파일을 실수로 수정하는걸 막기 위해 VSCode의 읽기 전용 옵션 추가

- File > Preferences > Settings > Workspace
  - Search settings 에서 Files: Readonly Include 검색
  - Add Pattern > `api/**/readonly-*` 입력한 후 OK 선택하면 다음의 폴더는 전부 VSCode에서 읽기 전용이 됨
